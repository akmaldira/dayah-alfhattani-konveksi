...
export default function PrintExpense() {
  ...
  const employeeLoanService = useQuery({
    queryKey: ["expense", fromDate, toDate],
    queryFn: async () => {
      const response = await getTransactionsAction(fromDate, toDate, "EXPENSE");
  ...
  function handleDownload(data: EmployeeSalaryTransaction[]) {
    ...
    saveAs(blob, `pengeluaran-${formatDate(new Date())}.xlsx`);
  }
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Download atau Cetak data pengeluaran
          </DialogDescription>
        </DialogHeader>
